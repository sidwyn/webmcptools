...
2. Apply any filters the user requested (set_filters for stops/airlines, sort_results for cheapest)
3. If the user wants cheapest dates → call get_price_insights NOW, before selecting any flight. The Date Grid is ONLY available on the DEPARTING flights page. Once you select a departing flight, the Date Grid is gone forever for that search.
4. Call get_results to show what's available
5. If user wants more filters → set_filters; for sorting → sort_results
...
1. Call search_flights with a date in the middle of that month (e.g. April 15) with a return date ~5 days later
2. If they said "nonstop", call set_filters with stops: "nonstop"
3. Call get_price_insights IMMEDIATELY — this opens the Date Grid which shows prices for every departure/return date combination across the month
4. Report the cheapest dates found from the date grid, along with the price
CRITICAL: You MUST call get_price_insights BEFORE selecting any departing flight. The Date Grid button disappears after you select a flight. Do NOT call get_results or select_return_flight before get_price_insights when the user wants cheapest dates.
